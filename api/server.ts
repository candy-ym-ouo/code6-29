...
import { gainXp, migrateActorProgression } from './progression.js';
...
type TourActor = Actor & {level:number;xp:number;stamina:number;fatigue:number};
type Tour = { id:string; name:string; seed:number; status:string; townIds:string[]; stopIndex:number; funds:number; reputation:number; inspiration:number; version:number; actors:TourActor[]; visited:string[]; clues:Record<string,string[]>; draft?:Draft; history:any[]; unlocked:string[] };
...
for (const tour of tours) {
  const savedActors: unknown[] = Array.isArray(tour.actors) ? tour.actors : [];
  if (!Array.isArray(tour.actors)) { tour.actors = []; saveMigrated = true; }
  tour.actors = savedActors.map((savedActor) => {
    const before = savedActor as Partial<TourActor> | null;
    const actor = migrateActorProgression(before ?? {}) as TourActor;
    if (before?.level !== actor.level || before?.xp !== actor.xp) saveMigrated = true;
    return actor;
  });
}
if (saveMigrated && existsSync(file)) writeFileSync(file, JSON.stringify(tours,null,2));
...
app.post('/api/v1/tours/:id/finale',(req,res)=>{const t=getTour(req,res);if(!t||!('id'in t))return;if(t.status!=='FINALE_READY')return fail(res,'INVALID_TOUR_STATE','终局尚未开放',409);const avg=t.history.slice(1).reduce((n,x)=>n+x.score,0)/Math.max(1,t.history.length-1);t.status='COMPLETED';t.version++;persist();ok(res,{ending:avg>=75?'万镇喝彩':avg>=55?'忠于舞台':'散场之后',average:Math.round(avg),tour:t})});
